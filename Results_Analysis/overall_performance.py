...
def compute_results(
    data_root: str, datasets: list[str], device: str = "cpu", use_clip: bool = True
):
    ...
    for name, ds in datasets_map.items():
        ...
        results_detection[name] = {
            ...
            "clip_global": evaluate_auroc(
                [x[1] for x in aurocs], [x[5] for x in aurocs]
            ),
...
def main():
    ...
    p.add_argument(
        "--no_clip",
        action="store_true",
        help="Disable CLIP scoring (useful for machines without clip installed)",
    )
    args = p.parse_args()
    results = compute_results(
        args.data_root, args.datasets, device=args.device, use_clip=not args.no_clip
    )
